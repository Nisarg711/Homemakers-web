...
#The results returned in sql results will be array/list of jsons, we need to format it
...
async def sql_execute(state:State):
...
graph_builder.add_node("sql_generate", sql_generate)
graph_builder.add_node("sql_execute", sql_execute)
graph_builder.add_node("router",router)
graph_builder.add_node("tool_node", tool_node)
graph_builder.add_conditional_edges("router", route_decision)
graph_builder.add_edge("sql_generate", "sql_execute")
graph_builder.add_conditional_edges("model", tools_router)
graph_builder.add_edge("tool_node", "model")
graph_builder.add_node("format_results", format_results)
graph_builder.add_edge("sql_execute", "format_results")
...
'''The below function gives output to user token by token, but now there are also
nodes to generate sql, so we will have to modify it so that user doesn't see those sql tokens'''
